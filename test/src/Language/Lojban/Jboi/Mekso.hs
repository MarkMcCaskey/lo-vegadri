...
{-|
Module     : Language.Lojban.Jboi.Mekso
Description: Representation of mathematical expressions
-}
module Language.Lojban.Jboi.Mekso where
...
vuhuMap = Map.fromAscList
  ...
  , ("gei", TernaryFunc (\(Lit (JboNum a))
                          (Lit (JboNum b))
                          (Lit (JboNum c)) -> Lit (JboNum (b * (a ^ c))))) --flip a and c NOTE: THIS NEEDS TO BE FIXED IN THE RESOLVER/FUNCTION CONSTRUCTOR
